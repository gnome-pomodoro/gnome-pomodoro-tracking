...
class Odoo(Plugin):
    ...
    def add_parse_args(self, kind):
        ...
        self.gpt.parse.add_argument("--username", action="store", dest="odoo_username")
        self.gpt.parse.add_argument("--password", action="store", dest="odoo_password")
        self.gpt.parse.add_argument("--database", action="store", dest="odoo_database")
        self.gpt.parse.add_argument("--url", action="store", dest="odoo_url")
    ...
    def common(self):
        return xmlrpc.client.ServerProxy(
            join_url(self.session.get("url", ""), "xmlrpc/2/common")
        )
    ...
    def models(self, model, method, domain, options=False):
        ...
        x = xmlrpc.client.ServerProxy(
            join_url(self.session.get("url"), "xmlrpc/2/object")
        )
        return x.execute_kw(
            self.session.get("database"),
    ...
    def cli(self, **kwargs):
        ...
        if (
            hasattr(params, "odoo_username")
            and params.odoo_username
            and hasattr(params, "odoo_password")
            and params.odoo_password
            and hasattr(params, "odoo_url")
            and params.odoo_url
            and hasattr(params, "odoo_database")
            and params.odoo_database
        ):
            self.session.update(
                {
            ...
            try:
                ...
                if params.set:
                    ...
                    if row:
                        ...
                        self.gpt.set_config(
                            self.name, "project_id", row.get("project_id")
                        )
                        self.gpt.set_config(
                            self.name, "project_name", row.get("project_name")
                        )
                        printtbl([row])
                    else:
