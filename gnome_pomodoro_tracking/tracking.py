...
class Tracking:
    ...
    def setup(self):
        ...
        self.parse = argparse.ArgumentParser(
            ...
            epilog="GNOME Pomodoro Tracking <https://gnomepomodoro.org>",
        )
        self.plugin = None
    ...
    def load_plugin(self):
        ...
        try:
            ...
            self.logger.critical(
                "In the file .gnome-pomodoro-tracking.conf no exists secction [settings]."
                "\nTry re-install for regenerate file config.\n"
            )
        except configparser.NoOptionError as e:
            self.logger.critical(
                "Exec the command gnome-pomodoro-tracking --plugin NAME for set.\n"
            )
    ...
    def add_parse_args(self):
        ...
        self.parse.add_argument(
            "-n", "--name", action="store", dest="name", help="Pomodoro name"
        )
        self.parse.add_argument(
            "-g", "--tag", action="store", dest="tag", help="Pomodoro tag"
        )
        self.parse.add_argument(
            "-r",
            "--restart",
        ...
        self.parse.add_argument(
            "--set", action="store", dest="set", help=argparse.SUPPRESS
        )
        self.parse.add_argument(
            "-e",
            "--time-entry",
    ...
    def cli(self):
        ...
        if not self.gnome_pomodoro():
            ...
            if params.name:
                if not len(self.pomodoro_config("name")) and not len(
                    self.pomodoro_config("start")
                ):
                    os.system("gnome-pomodoro --stop")
                    os.system("gnome-pomodoro --start --no-default-window")
            ...
            if params.status:
                items = [
                    {
                        "key": "Plugin",
                        "value": str(self.settings_config("plugin")).title(),
                    }
                ]
                dt_start = utils.now()
                ...
                items.append(
                    {
                        "key": "Elapsed",
                        "value": "{0:.2f} Min".format(
                            utils.time_elapsed(
                                dt_start, utils.now(), formatter="minutes"
                            )
                        ),
                    }
                )
    ...
    def add_time_entry(self, **kwargs):
        try:
            name = (
                ...
                or self.pomodoro_config("name")
                or self.pomodoro_config("type")
            )
    ...
    def gnome_pomodoro(self, params=None):
        ...
        # Stop timer
            ...
            "skip" in params.gp_trigger
            or "pause" in params.gp_trigger
            or "complete" in params.gp_trigger
        ):
            self.add_time_entry()
        return True
